        ...
        >
          <div>
            ...
            <p style={{ fontSize: 12, color: "var(--color-text-muted)", margin: "4px 0 0" }}>
              This month {countMonthEvents(ddls, cursor)} items due · click an empty date to view the timeline
            </p>
          </div>
...
function TimelinePill({ event, onClick, draggable }: { event: DdlItem; onClick: (e: React.MouseEvent) => void; draggable?: boolean }) {
  ...
  return (
    <button
      ...
      title={draggable ? `${event.taskName} (drag to another time slot)` : event.taskName}
    >
      <span
        style={{
          fontSize: 11,
  ...
  return (
    ...
    >
      ...
      <p style={{ fontSize: 12, color: "var(--color-text-muted)", margin: "0 0 16px" }}>
        Tell Butler your schedule, upload course files, or create one manually
      </p>
      <PrimaryBtn onClick={onCreate} icon={<Plus size={14} />} label="New Event" />
    </div>
  );
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div>
              ...
              <p style={{ fontSize: 12, color: "var(--color-text-muted)", margin: "2px 0 0" }}>
                {weekLabel} · {eventsInWeek.length} events
              </p>
            </div>
          </div>
          <button
                ...
                {weekDays.map((d) => {
                  ...
                  return (
                    <div
                      ...
                      title={onCreateAt ? `Create event · ${d.iso} ${String(hour).padStart(2, "0")}:00` : ""}
                    >
                      {cellEvents.map((e) => (
                        <button
                          key={e.id}
