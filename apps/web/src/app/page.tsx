...
function ButlerApp() {
  ...
  // Notes 数据：Express API 持久化，页面内保留 React state
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const sessRows = chatRows.sessions as ChatSession[];
  ...
  // 改动同步：每次 ddls 变化，整体替换后端 tasks 集合
  useEffect(() => {
  ...
  // 持久化：聊天历史走 Express API + MongoDB；pipeline/confirm 不持久化
  useEffect(() => {
    if (!hydrated) return;
    ...
    (async () => {
      try {
        await ReplaceChatHistoryByApi({ sessions, messages: persistable });
      } catch (e) {
      ...
      await streamChat({
        ...
        callbacks: {
          ...
          onError: (err) => {
            setMessages((prev) => [
              ...prev,
              { id: uid(), sessionId: sid, role: "assistant", content: `❌ Error: ${err.message}`, isError: true, timestamp: new Date() },
            ]);
          },
        },
      });
    ...
    const check = () => {
      ...
      for (const d of ddlsRef.current) {
        ...
        if (dueTs >= now && dueTs <= horizon) {
          ...
          // Toast 一定弹
          toast.warning(`⏰ ${d.taskName} is due in about ${hoursLeft} hour(s)`, {
            duration: 10000,
            action: { label: "View", onClick: () => handleJumpToTask(d.id) },
          });
          // 浏览器 Notification(可选,需授权)
          if (typeof window !== "undefined" && "Notification" in window && Notification.permission === "granted") {
            try {
              new Notification("Butler · Upcoming deadline", {
                body: `${d.taskName} · ${d.dueDate} ${d.dueTime || "23:59"} (about ${hoursLeft}h)`,
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      newly.forEach((a, i) => {
        setTimeout(() => {
          toast.success(`${a.emoji} Achievement unlocked: ${a.label} - ${a.desc}`, { duration: 6000 });
        }, i * 500);
      });
  ...
  // Epic 2 global shortcuts: Ctrl+N creates an item in the current tab; ? opens help; Esc closes overlays.
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      // Ctrl+N creates an item in the current tab. Meta+N is still accepted for Mac compatibility.
  ...
  const handleExportIcs = useCallback(async () => {
    ...
    if (r.count === 0) {
      ...
      toast.success(`Exported ${r.count} event(s) to .ics`);
    }
  }, [toast]);
  ...
  const handleImportJson = useCallback(async () => {
    ...
    if (r.ok !== true) { toast.error("Import failed: " + r.error); return; }
    setDdls(r.merged);
    toast.success(`Import complete: added ${r.added}, updated ${r.updated}`);
  }, [toast]);
