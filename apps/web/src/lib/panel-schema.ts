...
export function validateSpec(raw: unknown): ParseResult {
  ...
  if (typeof o.title !== "string" || !o.title.trim()) return { ok: false, error: "missing title" };
  if (!Array.isArray(o.sources)) return { ok: false, error: "sources must be an array" };
  if (!Array.isArray(o.blocks)) return { ok: false, error: "blocks must be an array" };
  // SEC-13：上限保护，防超大 spec（AI/导入/篡改）撑爆渲染或携带巨型载荷
  if (o.sources.length > SPEC_LIMITS.maxSources) return { ok: false, error: `too many data sources (>${SPEC_LIMITS.maxSources})` };
  if (o.blocks.length > SPEC_LIMITS.maxBlocks) return { ok: false, error: `too many blocks (>${SPEC_LIMITS.maxBlocks})` };
  for (const s of o.sources as DataSource[]) {
    if (s && typeof s === "object" && Array.isArray(s.transforms) && s.transforms.length > SPEC_LIMITS.maxTransforms) {
      return { ok: false, error: `too many data-source transforms (>${SPEC_LIMITS.maxTransforms})` };
    }
  }
  ...
  for (const s of o.sources as DataSource[]) {
    if (s.kind === "http" && typeof s.url !== "string") return { ok: false, error: `HTTP data source ${s.id} is missing url` };
    if (s.kind === "local" && !s.dataset) return { ok: false, error: `Local data source ${s.id} is missing dataset` };
  }
  ...
  for (const b of o.blocks as Block[]) {
    if (!b.type) return { ok: false, error: `block ${b.id} is missing type` };
  }
...
export const SAMPLE_SPEC: GeneratedPanelSpec = {
  version: 1,
  title: "Crypto Top 10 · Live Prices",
  emoji: "📈",
  description: "CoinGecko public API through the backend connector proxy, demonstrating the data-source and block engine.",
  sources: [
    {
      id: "coins",
      kind: "http",
  ...
  blocks: [
    {
      ...
      metrics: [
        { label: "Coins", sourceId: "coins", agg: "count" },
        { label: "Total market cap", sourceId: "coins", agg: "sum", field: "market_cap", format: "currency" },
        { label: "Highest price", sourceId: "coins", agg: "max", field: "current_price", format: "currency" },
      ],
    },
    {
      id: "bar-price",
    ...
    },
    {
      ...
      columns: [
        ...
        { key: "current_price", label: "Current price", format: "currency" },
