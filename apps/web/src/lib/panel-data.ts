...
export function resolveStat(metric: PanelMetric | undefined, ctx: PanelDataCtx): { value: number; label: string } {
  const t = todayIso();
  switch (metric) {
    case "tasks-total": return { value: ctx.ddls.length, label: "Total tasks" };
    case "tasks-done": return { value: ctx.ddls.filter(isDone).length, label: "Completed tasks" };
    case "tasks-active": return { value: ctx.ddls.filter((d) => !isDone(d)).length, label: "Active tasks" };
    case "tasks-today": return { value: ctx.ddls.filter((d) => d.dueDate === t && !isDone(d)).length, label: "Due today" };
    case "notes-total": return { value: ctx.notes.length, label: "Notes" };
    case "streak-current": return { value: ctx.streakCurrent, label: "Current streak" };
    case "streak-longest": return { value: ctx.streakLongest, label: "Longest streak" };
    default: return { value: ctx.ddls.filter((d) => !isDone(d)).length, label: "Active tasks" };
  }
}
  ...
  switch (metric) {
    ...
    case "tasks-by-source": {
      ...
      for (const d of ctx.ddls) {
        const key = d.source || "Uncategorized";
