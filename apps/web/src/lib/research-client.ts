...
export async function runResearch(
  ...
  try {
    const r = await postJSON<{ ok: boolean; plan?: ResearchPlan; error?: string }>(
      "/express-api/research/plan",
      { goal },
      signal,
    );
    if (r.ok !== true || !r.plan) {
      onProgress({ phase: "error", squads: [], error: r.error ?? "Decomposition failed" });
      return { ok: false, error: r.error ?? "Decomposition failed" };
    }
    plan = r.plan;
  } catch (e) {
