...
export function assembleSpec(
  ...
  blocks.push({
    ...
    markdown: `> Research synthesis from parallel teams. AI-generated data should be verified against real sources.\n\n**${plan.squads.length}** team(s) completed the investigation:\n\n${squadList}`,
  });
  ...
  plan.squads.forEach((squad, i) => {
    ...
    if (!f) {
      blocks.push({
        ...
        type: "markdown",
        title: squad.title,
        markdown: "This team returned no result.",
      });
      return;
    }
    ...
    if (f.summary && f.summary.trim()) {
      blocks.push({
        id: `sq-${i}-sum`,
        type: "markdown",
        title: hasData ? `${squad.title} · Key Points` : squad.title,
        markdown: f.summary,
        span: hasData ? 1 : 2,
      });
    }
  ...
  return {
    ...
    title: plan.title,
    emoji: plan.emoji,
    description: "Generated from parallel research teams. AI-synthesized data can be connected to real data sources in Apps.",
    sources,
    blocks,
  };
}
