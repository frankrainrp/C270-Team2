...
export async function getAllRecurring(): Promise<RecurringTask[]> {
  const list = await CallRecurringApi<RecurringTask[]>("/recurring");
  return list.sort((a, b) => b.createdAt - a.createdAt);
}

export async function putRecurring(routine: RecurringTask): Promise<void> {
  await CallRecurringApi<RecurringTask>(`/recurring/${encodeURIComponent(routine.id)}`, {
    method: "PUT",
...
export async function bulkPutRecurring(routines: RecurringTask[]): Promise<void> {
  ...
  await CallRecurringApi<RecurringTask[]>("/recurring/replace/all", {
    method: "PUT",
...
export async function deleteRecurring(id: string): Promise<void> {
  await CallRecurringApi(`/recurring/${encodeURIComponent(id)}`, { method: "DELETE" });
  dispatchChange();
}
...
async function CallRecurringApi<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  if (!json.ok) throw new Error(json.error || "recurring request failed");
