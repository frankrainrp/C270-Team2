...
const json = (label, data) => ok(`${label}\n\n\`\`\`json\n${JSON.stringify(data, null, 2)}\n\`\`\``);
...
server.registerTool("list_notes", {
    ...
    let rows = notes;
    if (tag)
        rows = rows.filter((n) => n.tags?.includes(tag));
    if (pinnedOnly)
...
server.registerTool("search_notes", {
    ...
    if (!hits.length)
        return ok(`No notes match "${query}".`);
...
server.registerTool("create_note", {
    ...
    const now = Date.now();
    const note = { id: newId("note"), title, content, tags, pinned, createdAt: now, updatedAt: now };
...
server.registerTool("list_tasks", {
    ...
    if (source)
        rows = rows.filter((t) => t.source === source);
    if (tag)
        rows = rows.filter((t) => t.tags?.includes(tag));
    if (dueBefore)
        rows = rows.filter((t) => t.dueDate && t.dueDate <= dueBefore);
...
server.registerTool("list_events", {
    ...
    if (!rows.length)
        return ok(`No events between ${from} and ${to}.`);
    const lines = rows.map((t) => `📅 ${t.dueDate}${t.dueTime ? " " + t.dueTime : ""} — ${t.taskName} [${t.id}]`);
...
server.registerTool("toggle_workflow", {
    ...
    if (!result)
        return ok(`No workflow with id ${id}.`);
